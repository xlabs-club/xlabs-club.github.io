...
- **不推荐**在每个业务集群各部署一套 ArgoCD——失去集中可视化、AppProject 隔离已能满足多租户需求。
...
### 7.10 GitOps Bridge——基础设施与应用的桥梁

ArgoCD 管理的是 Kubernetes 内的资源，但集群本身（VPC、EKS/GKE/AKS、IAM 角色、KMS 密钥）不在它的管辖范围内。GitOps Bridge 是 2025-2026 年的行业标准模式——**Terraform/Crossplane 负责基础设施创建，ArgoCD 接管集群内所有资源**：

```
阶段 1-2: Terraform                     阶段 3-4: ArgoCD
────────────────────                    ────────────────
VPC                                      监控系统（Prometheus/Grafana）
EKS/GKE/AKS 集群                          Ingress Controller（Nginx/Traefik）
Node Groups                              Cert-Manager
IAM Roles (IRSA)                         Policy Engine（Kyverno）
KMS Keys                                 所有业务应用
└── ArgoCD Helm 安装 (Bootstrap)          External Secrets
    └── 创建 Root Application ──▶          GitOps 仓库
```

**关键原则**：Terraform 只触碰集群**一次**——在 Bootstrap 阶段安装 ArgoCD 并创建指向 GitOps 仓库的 Root Application。之后全部由 ArgoCD 自我管理，包括 ArgoCD 自身的升级和配置变更。

```hcl
# Terraform Bootstrap —— 安装 ArgoCD 并创建 Root Application
resource "helm_release" "argocd" {
  ...
  values = [<<-YAML
    configs:
      secret:
        argocdServerAdminPassword: ${bcrypt_hash}
      params:
...
# Root Application —— ArgoCD 自管理的入口
...
**ArgoCD 接入新集群的标准流程**（集群生命周期见 7.11）：

```bash
# 1. Terraform 创建新集群（或手动）
# 2. 将新集群添加到 ArgoCD
argocd cluster add <context> --name <alias> --label env=production,region=us-east

# 3. ApplicationSet 检测到新标签匹配 → 自动部署所有匹配的 Application
# 无需修改任何 YAML
```
