...
| 资源隔离 | 依赖 K8s RBAC | 依赖 K8s RBAC | AppProject + RBAC + SSO |
...
多集群部署有四种成熟模式，选型取决于团队规模、集群数量和合规需求：

| 模式 | 架构 | 适用规模 | 核心权衡 |
|------|------|----------|----------|
| **Hub-Spoke** | 1 个管控集群运行 ArgoCD，管理所有业务集群 | < 50 集群，< 3000 应用 | 集中管控简单，但管控集群是单点 |
| **去中心化** | 每个业务集群独立运行 ArgoCD | 独立团队、隔离要求极高 | 无单点故障，但失去集中可视化 |
| **混合模式（推荐）** | 管控集群 + 团队级 AppProject 隔离 | 大多数企业 | 平衡集中治理和自治 |
| **Agent 模式** | 集中 API/UI，Controller 以 Agent 分散到各集群 | > 50 集群，超大规模 | 集中可视化 + 无单点瓶颈（ArgoCD Agent / OpenShift GitOps 1.19+） |

本方案推荐**混合模式**——管控集群集中运行 ArgoCD，通过 AppProject 和 RBAC 实现团队隔离，目标集群通过标签选择器自动匹配：
...
│  └──────────┬──────────┘  │
└─────────────┼─────────────┘
              │ ApplicationSet (Cluster Label Selector)
     ┌────────┼────────┬──────────┐
     ▼        ▼        ▼          ▼
┌────────┐ ┌──────┐ ┌──────┐ ┌──────────┐
│ Dev    │ │Stg   │ │Prod  │ │Edge/     │
│ env=dev│ │env=stg│ │env=prod│ │env=edge  │
└────────┘ └──────┘ └──────┘ └──────────┘
```

关键设计决策：
- ArgoCD 组件部署在独立的管控集群，**绝不与生产集群共用**——如果生产集群故障，部署系统也必须正常工作。
- 目标集群通过 `argocd cluster add` 注册并打标签，凭据存储为 Kubernetes Secret。
...
> **selfHeal 策略的选择**：业界有两种观点——`selfHeal: true`（自动修正漂移）vs `selfHeal: false`（保留手动修复窗口）——二者各有道理。本方案推荐 **生产环境 selfHeal: true**，原因是：(1) selfHeal 的修正间隔约 3 分钟，紧急情况下足够 kubectl 手动止血然后提交 Git commit；(2) 如果关闭 selfHeal，手动变更会永久残留，累积到下次部署时引发意外。配套的紧急流程见第十五章。

### 4.3 AppProject——资源与权限边界
...
- `SecretStore`（命名空间级）：不同 namespace 可连接不同 Provider
- `ClusterSecretStore`（集群级）：多个 namespace 共用同一 Provider 连接
...
### 6.4 SOPS——加密后存入 Git（Zero 外部依赖方案）

SOPS 是 Mozilla 开源的加密编辑器，通过 Age 或云 KMS 对 YAML/JSON 文件做字段级加密——只加密 values，保留 key 和结构。与 ESO 相比，**SOPS 不需要任何外部 Secret Store**，所有依赖仅为集群内一个 Age 私钥，是最 "GitOps 纯粹" 的 Secret 方案。

**核心优势**：

- **零外部依赖**——不引入 Secret Manager / Vault，架构最简单
- **加密文件直接存 Git**——`git log` 能看到完整的 Secret 变更历史（变更了什么 key、什么时候、谁操作的），ESO 做不到这一点——ExternalSecret CRD 在 Git 中只是一个引用字符串，真实的 Secret 变更历史分散在云 Provider 的审计日志里
- **CI 友好**——`sops -d` 解密后直接 pipe 给 `kubectl`，本地调试方便
- **字段级加密**——YAML 结构保留可读，`sops my-secret.yaml` 直接在编辑器中打开解密后的明文

**核心代价**：

- ArgoCD 集成需要 Config Management Plugin（CMP），比 ESO 的原生兼容多一步配置
- 密钥管理是用户责任——私钥丢了，所有加密文件永久不可解密
...
# age1xxxx... ← 这个值配在 .sops.yaml 中

# 私钥（绝对保密！用于解密）
cat age-key.txt | grep 'AGE-SECRET-KEY-'
```

**Step 2：配置 .sops.yaml**

在 GitOps 仓库根目录创建 `.sops.yaml`：

```yaml
creation_rules:
  # 所有 secret.enc.yaml 文件自动使用此公钥加密
  - path_regex: .*secret(s)?\.enc\.(yaml|json)$
    age: age1xxxxyour-public-key-herexxxxxxxxxxxxxxxxxxxxxx

  # 不同环境可用不同的 Key（可选）
  - path_regex: overlays/prod/.*\.enc\.yaml$
    age: age1prod-key-here
```

此后 `sops my-secret.enc.yaml` 会自动匹配规则，无需每次指定 `--age`。

**Step 3：日常加密/编辑**

```bash
# 创建加密文件（直接编辑，sops 自动加密保存）
...
**Step 4：ArgoCD CMP 集成**（关键步骤）

这是 SOPS + ArgoCD 的核心——通过 `ksops` CMP 让 ArgoCD 在渲染 Kustomize 时自动解密 SOPS 文件：
...
data:
  configManagementPlugins: |
    - name: ksops
      init:
        command: [sh, -c]
        args: ["echo 'ksops is ready'"]
      generate:
        command: [sh, -c]
        args:
          - |
            kustomize build --enable-alpha-plugins --enable-exec \
              --network $ARGOCD_ENV_SOURCE_PATH
```

自定义 ArgoCD Repo Server 镜像——注入 `sops`、`age`、`ksops` 二进制：

```dockerfile
# argocd-repo-server.Dockerfile
FROM argoproj/argocd:latest

# 安装 age
ARG AGE_VERSION=v1.2.1
RUN curl -Lo /usr/local/bin/age-keygen \
    https://github.com/FiloSottile/age/releases/download/${AGE_VERSION}/age-keygen-linux-amd64 \
    && chmod +x /usr/local/bin/age-keygen \
    && curl -Lo /usr/local/bin/age \
    https://github.com/FiloSottile/age/releases/download/${AGE_VERSION}/age-linux-amd64 \
...
# 安装 ksops（Kustomize SOPS 插件）
ARG KSOPS_VERSION=v4.3.2
RUN curl -Lo /usr/local/bin/ksops \
    https://github.com/viaduct-ai/kustomize-sops/releases/download/${KSOPS_VERSION}/ksops_${KSOPS_VERSION}_Linux_x86_64.tar.gz \
    && tar -xzf /usr/local/bin/ksops -C /usr/local/bin/ \
    && chmod +x /usr/local/bin/kustomize-sops
```
...
```yaml
# 在 argocd-repo-server Deployment 中挂载私钥
spec:
  template:
    spec:
      containers:
...
**Step 5：Kustomize 中使用 SOPS**

```yaml
# workloads/myapp/base/kustomization.yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
resources:
  - deployment.yaml
...
generators:
  - secrets-generator.yaml      # KSOPS 生成器引用
```

```yaml
# workloads/myapp/base/secrets-generator.yaml
apiVersion: viaduct.ai/v1
kind: ksops
metadata:
  name: secret-generator
files:
  - secret.enc.yaml              # SOPS 加密的文件
```

这样 ArgoCD 渲染时自动解密 `secret.enc.yaml`→生成 K8s Secret→应用到集群。

#### 6.4.2 多环境 + 不同密钥

不同环境使用不同 Age Key，防止 dev 环境密钥泄露影响 prod：
...
在 ArgoCD 中每个环境挂载各自的 Age Key Secret。
...
# 更新所有加密文件的 Recipient（用新公钥重新加密）
sops updatekeys -y my-secret.enc.yaml

# 更新 ArgoCD 中的私钥
kubectl create secret generic sops-age-key \
  --from-file=age-key.txt=age-key-new.txt \
  -n argocd --dry-run=client -o yaml | kubectl apply -f -

# 旧私钥此时可安全销毁
```

#### 6.4.4 SOPS vs ESO 实际体验对比

| 维度 | SOPS | ESO |
|------|------|-----|
| **架构复杂度** | 极简（1 个二进制 + 1 个密钥文件） | 中等（ESO Controller + SecretStore + Provider 配置） |
| **Secret 变更可追溯性** | ✅ 完整 Git 历史（哪个字段、什么值变成什么值、谁改的） | ⚠️ Git 中只有引用字符串，Secret 真实变更是"外部事件" |
| **Secret 自动轮转** | ❌ 手动——重新加密 + commit | ✅ `refreshInterval` 自动 |
| **本地调试** | ✅ `sops -d file | kubectl apply -f -` 一条命令 | 需要连接外部 Provider |
| **多人协作** | ⚠️ 每个人需要公钥加密；加密文件冲突需手动解决 | ✅ 无冲突——ESO CRD 只是引用 |
| **灾难恢复** | ⚠️ 私钥必须备份，丢失则所有加密文件永久不可恢复 | ✅ 依赖外部 Provider 的备份 |
| **CI 明文风险** | ⚠️ CI Runner 解密时可短暂看到明文 | 无——CI 不管 Secret |
...
- 团队规模中等到大，但希望保持架构极简
- **重视 Secret 变更的 Git 可追溯性**——SOPS 是唯一能在 Git 中看到完整 Secret 变迁历史的方案
- 有成熟的密钥管理习惯（私钥备份、轮转）
- Secret 更新频率低（月级别，不是每天自动轮转）
- 愿意维护一个自定义 ArgoCD Repo Server 镜像

**不推荐 SOPS 的场景**：

- 需要 Secret 自动轮转（每天/每小时换数据库密码）——这是 Vault 和 ESO 的战场
- 多人频繁修改同一批 Secret 文件——加密文件 merge conflict 不好解决
- 合规强制要求"Secret 不进 Git"（即使加密也不行）——用 Vault CSI 方案

**个人评估**：SOPS 的正确使用体验并不差——`sops my-secret.enc.yaml` 打开就是明文编辑、保存自动加密、`git diff` 能看到明文 diff（因为 SOPS 文件保留了 YAML key）。与 ESO 的 "面向配置" 不同，SOPS 是 "面向文件"——对于习惯 Git 工作流的团队，这可能比 ESO 更自然。ArgoCD CMP 配置是一次性成本，后续所有文件共享同一套解密机制。
...
PostSync 不适合执行耗时操作。**数据归整、缓存预热、完整回归测试**等长时间任务，通过 Argo Events 监听 Sync 完成事件触发 Argo Workflow 执行（详见第十二章）：
...
在生产环境中，集群不是一次性创建的——有新集群上线、有集群下线、有集群需要凭据轮转。以下给出标准化管理流程。
...
argocd cluster add <context-name> \
  --name <cluster-alias> \
  --label env=<dev|staging|prod> \
  --label region=<region> \
  --label cloud=<aws|gcp|azure|onprem> \
  --label team=<team-name> \
  --label release-channel=<rolling|pinned>
...
| 标签 Key | 必填 | 可选值 | 用途 |
|----------|------|--------|------|
| `env` | ✅ | `dev`, `staging`, `production` | 分环境策略匹配 |
| `region` | ✅ | 任意 | 区域感知分发 |
| `cloud` | ✅ | `aws`, `gcp`, `azure`, `onprem` | 云差异配置区分 |
| `release-channel` | ✅ | `rolling`, `pinned` | 7.5 节多云异步发布 |
| `team` | 否 | 任意 | 可选团队归属标签 |

**集群健康监控**：

```yaml
# Prometheus 指标: 集群连接状态
argocd_cluster_connection_status{cluster="prod-us-east-1"} 1  # 1=正常, 0=断开
...
- alert: ArgoCDClusterDisconnected
  ...
  for: 5m
  annotations:
    summary: "ArgoCD 无法连接到集群 {{ $labels.cluster }}"
```
...
```bash
# 1. 确认集群无残留工作负载
argocd app list -o json | jq '.[] | select(.spec.destination.server=="<cluster-url>") | .metadata.name'

# 2. 移除所有指向该集群的 Application（或让 prune 自动执行）
# 3. 从 ArgoCD 移除集群
argocd cluster rm <cluster-url>

# 4. 确认 ApplicationSet 不再自动创建指向该集群的 Application
```

**凭据轮转**：

```yaml
# 集群凭据（kubeconfig）存储在 Secret 中，定期轮转
apiVersion: v1
kind: Secret
metadata:
  ...
  labels:
    argocd.argoproj.io/secret-type: cluster
  annotations:
    argocd.argoproj.io/credential-rotation-at: "2026-10-01"  # 提醒下次轮转日期
```

> 凭据轮转后无需重启任何组件——ArgoCD 动态从 Secret 读取。
...
**审计存储建议**：将 JSON 日志通过 Fluentd/Vector 转至 Elasticsearch 或 ClickHouse，保留不少于 90 天。合规场景下建议在日志基础上补充 `argocd admin export` 定期全量备份（见第十八章灾难恢复）。
...
### 12.4 WorkflowTemplate——可复用的部署后任务
...
### 15.2 方式二：argocd app rollback（紧急回滚）
...
ArgoCD 自身也是一种"基础设施"——如果管控集群故障，所有 Git→集群的同步能力丧失。但**已部署的服务不受影响**——ArgoCD 是部署工具而非运行时依赖。

### 18.0 ArgoCD 宕机影响分析

这是方案评审中最常见的问题之一：**ArgoCD 挂了，线上服务会停吗？**

**答案：不会。** ArgoCD 的职责是"让集群状态向 Git 靠拢"，它不参与服务运行时的任何环节。各组件宕机的影响范围：

| 组件宕机 | 已部署服务 | 新部署 | 漂移检测 | WebUI/CLI |
|----------|-----------|--------|----------|-----------|
| Application Controller | ✅ 正常运行 | ❌ 停止同步 | ❌ 停止 | ✅ 可查看（API 还活着） |
| API Server | ✅ 正常运行 | ❌ 无法触发手动同步 | ✅ 正常运行 | ❌ UI/CLI 不可用 |
| Repo Server | ✅ 正常运行 | ❌ 无法渲染新清单 | ✅ 正常运行（使用缓存） | ✅ 可查看 |
| Redis | ✅ 正常运行 | ⚠️ 降级（无缓存，需重新克隆仓库） | ⚠️ 变慢 | ✅ 可查看 |
| **全部组件同时宕机** | ✅ **正常运行** | ❌ 停止 | ❌ 停止 | ❌ 不可用 |

**各场景的影响和应对**：

| 场景 | 服务影响 | 部署影响 | 持续时间 | 应对策略 |
|------|----------|----------|----------|----------|
| Controller 重启（1-2 min） | 无 | 瞬间（Leader Election 超时后备机接管） | < 2 min | HA 部署自动恢复 |
| 管控集群临时故障（< 30 min） | 无 | 新部署排队等待 | < 30 min | 等待恢复，业务集群不受影响 |
| 管控集群长时间故障（> 1 h） | 无 | 需紧急处理 | > 1 h | 见 18.2 恢复流程 |
| 管控集群完全丢失 | 无 | 全部部署能力丧失 | 永久 | 在新集群重建 ArgoCD + 从 Git/备份恢复 |

**紧急情况下没有 ArgoCD 如何部署？**

如果管控集群故障且需要立即部署（P0 修复），业务集群本身具备完整的 `kubectl` 能力——ArgoCD 的缺失不影响直接操作：

```bash
# 紧急方案：直接 kubectl（需记录事后补 Git commit）
kubectl --context=prod set image deployment/myapp myapp=registry.example.com/myapp:v9.9.6-hotfix
# 事后恢复 ArgoCD 后，selfHeal 会把状态拉回 Git 声明
# 所以止血后必须立刻提交 Git PR
```

> 这个流程与 15.6 节的紧急变更 SOP 一致——ArgoCD 宕机只是把这个 3 分钟窗口拉长了。
